...
// TC_HomePage_1
    ...
    public void userChangesCurrency(String currency) {
        ...
        H.clickOnCurrencyButton();

        switch (currency.toLowerCase()) {
            case "€ euro":
                H.clickOnEuroCurrency();
                break;
            case "£ pound sterling":
                H.clickOnPoundStrCurrency();
                break;
            case "$ us dollar":
                H.clickOnUsDollarCurrency();
                break;
        }

        // explicit wait
        WebDriverWait wait = new WebDriverWait(Hooks.driver, Duration.ofSeconds(5));
        wait.until(ExpectedConditions.not(ExpectedConditions.textToBePresentInElementLocated(H.P_Prices, "$")));
    }
    ...
    HomePage H17 = new HomePage(Hooks.driver);
        H17.verifyUserIsLoggedIn();
        Assert.assertTrue(H17.verifyUserIsLoggedIn(),"User is not logged in.");
    }
    ...
    public void productAddedToWishlist() {
        HomePage H17 = new HomePage(Hooks.driver);
        Assert.assertTrue(H17.wishlistHasProducts());
    }
    ...
    public void redirectedToWishlist() {
        Assert.assertTrue(Hooks.driver.getTitle().contains("Wish List"));
    }
